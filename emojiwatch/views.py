...
_UNRECOGNIZED_JSON_BODY_ERR = u'unrecognized JSON structure from request body'
...
def event_hook_handler(
    ...
    try:
        ...
        LOGGER.info(u'unable to parse JSON from request body')
        truncate_len = 1024
        half_truncate_len = truncate_len >> 1

        if len(request.body) > truncate_len:
            LOGGER.debug(u'%r', request.body[:half_truncate_len] + b'<...>' + request.body[-half_truncate_len:])
        else:
            LOGGER.debug(u'%r', request.body)

        return d_http.HttpResponseBadRequest()

    try:
        try:
            verification_token = payload_data[u'token']
        except (KeyError, TypeError):
            verification_token = None
            LOGGER.info(_UNRECOGNIZED_JSON_BODY_ERR + u" (missing 'token')")  # pylint: disable=logging-not-lazy

        if not verification_token \
                or verification_token != SLACK_VERIFICATION_TOKEN:
            raise RequestPayloadValidationError(
                message=u'bad verification token',
                response=d_http.HttpResponseForbidden(),
            )

        try:
            call_type = payload_data[u'type']
        except KeyError:
            raise RequestPayloadValidationError(
                message=_UNRECOGNIZED_JSON_BODY_ERR + u" (missing 'type')",
            )

        if call_type == u'url_verification':
            try:
                challenge = payload_data[u'challenge']
            except KeyError:
                raise RequestPayloadValidationError(
                    message=_UNRECOGNIZED_JSON_BODY_ERR + u" (missing 'challenge')",
                )

            if not isinstance(challenge, str) \
                    or len(challenge) > _CHALLENGE_MAX_LEN:
                raise RequestPayloadValidationError(
                    message=_UNRECOGNIZED_JSON_BODY_ERR + u" (unrecognized challenge)",
                )

            return d_http.HttpResponse(challenge, content_type='text/plain')

        if call_type != u'event_callback':
            raise RequestPayloadValidationError(
                message=u'unrecognized call type',
            )

        try:
            event = payload_data.get(u'event', {})
            event_type = event[u'type']
            event_subtype = event[u'subtype']
            team_id = payload_data[u'team_id']
        ...
        if not isinstance(event_type, str) \
            ...
            raise RequestPayloadValidationError(
                message=_UNRECOGNIZED_JSON_BODY_ERR + u' (unrecognized event type)',
            )
        ...
        try:
            ...
            raise RequestPayloadValidationError(
                message=_UNRECOGNIZED_JSON_BODY_ERR + u' (unrecognized event subtype)',
            )
        ...
        if not isinstance(team_id, str) \
            ...
            raise RequestPayloadValidationError(
                message=_UNRECOGNIZED_JSON_BODY_ERR + u' (unrecognized team_id)',
            )
        ...
        if team is None:
            raise RequestPayloadValidationError(
                message=u'no such team ({})'.format(team_id),
            )
        ...
        try:
            ...
            if exc.args == ('invalid_auth',):
                raise RequestPayloadValidationError(
                    message=u'call to Slack API failed auth',
                    response=d_http.HttpResponseForbidden(),
                )

            # Log, but otherwise ignore errors from our callbacks to Slack's
            # API
            LOGGER.info(u'falled call to Slack')
...
def _handle_add(
    ...
    if not isinstance(emoji_name, str) \
        ...
        raise RequestPayloadValidationError(
            message=_UNRECOGNIZED_JSON_BODY_ERR + u' (unrecognized event name)',
        )

    if not isinstance(emoji_url, str) \
            or len(emoji_url) > _EMOJI_URL_MAX_LEN:
        raise RequestPayloadValidationError(
            message=_UNRECOGNIZED_JSON_BODY_ERR + u' (unrecognized event value)',
        )

    if emoji_url:
        attachments = [  # needs to be a list, not a tuple
            {
                'fallback': u'<{}>'.format(emoji_url),
                'image_url': emoji_url,
            },
    ...
    return team.slack.chat.post_message(
        team.channel_id,
        html.escape(u'added `:{}:`').format(emoji_name),
...
def _handle_remove(
    ...
    if not isinstance(emoji_names, list):
        raise RequestPayloadValidationError(
            message=_UNRECOGNIZED_JSON_BODY_ERR + u' (unrecognized event names)',
        )
    ...
    return team.slack.chat.post_message(
        team.channel_id,
        html.escape(u'removed {}{}').format(u', '.join(u'`:{}:`'.format(name) for name in emoji_names[:_EMOJIS_MAX_LEN]), u'...' if too_many else u''),
    )
